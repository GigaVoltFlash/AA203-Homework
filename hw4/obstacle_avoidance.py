...
def signed_distances(s, centers, radii):
    ...
    d = jnp.linalg.norm(jnp.reshape(s[0:2], (-1, 2)) - centers, axis=1) - radii 
...
def scp_iteration(f, d, s0, s_goal, s_prev, u_prev, P, Q, R):
    ...
    for k in range(N):
        ...
        constraints.append(Ad[k] @ s_cvx[k] + cd[k] >= 0)
        constraints.append(s_cvx[k+1] == Af[k] @ s_cvx[k] + Bf[k] @ u_cvx[k] + cf[k])
    constraints.append(Ad[-1] @ s_cvx[-1] + cd[-1] >= 0)
    # END PART (e) ############################################################
...
for t in tqdm(range(T)):
    ...
    s_mpc[t], u_mpc[t] = solve_obstacle_avoidance_scp(f, d, s, s_goal, N, P, Q, R, eps, N_scp, s_init, u_init, False)

    # Push the state `s` forward in time with a closed-loop MPC input
    # Hint: You should call `f` here
    s = f(s_mpc[t, 0], u_mpc[t, 0])
